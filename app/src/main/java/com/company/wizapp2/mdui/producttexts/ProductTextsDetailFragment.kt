...
import com.company.wizapp2.databinding.ProducttextsDetailBinding
...
/**
 * A fragment representing a single ProductText detail screen. This fragment is either contained in an
 * ProductTextsListActivity in two-pane mode (on tablets) or an ProductTextsDetailActivity (on phone).
 */
class ProductTextsDetailFragment : Fragment() {
    /* Generated data binding class based on layout file */
    private lateinit var binding: ProducttextsDetailBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if (bundle != null && bundle.containsKey(BundleKeys.ENTITY_INSTANCE)) {
            productText = bundle.getParcelable(BundleKeys.ENTITY_INSTANCE)!!
            if (this.activity is ProductTextsListActivity) {
                activityTitle = productText.entityType.localName
                activity.invalidateOptionsMenu()
            } else {
                activity.title = productText.entityType.localName
            }
            setupObjectHeader(productText)
        }

        // In two pane mode, ProductTextsListActivity is already receiving delete completion callback
     ...
     *
     * @param [productText] to be displayed in ObjectHeader
     */
    private fun setupObjectHeader(productText: ProductText) {
        ...
        objectHeader?.subheadline = EntityKeyUtil.getOptionalEntityKey(productText)
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = ProducttextsDetailBinding.inflate(inflater, container, false)
