...
import com.company.wizapp2.databinding.ProducttextsCreateUpdateBinding
...
import kotlinx.android.synthetic.main.producttexts_create_update.*
 ...
 */
class ProductTextsCreateFragment : Fragment() {
    /* productText to be created or updated */
    private lateinit var productText: ProductText

    /* DataBinding generated class */
    private lateinit var binding: ProducttextsCreateUpdateBinding
    ...
    private val isProductTextValid: Boolean
        get() {
            val linearLayout: LinearLayout = create_update_producttext
            var isValid = true
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            ...
            productText = if (UIConstants.OP_CREATE == operation) {
                createProductText()
            } else {
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        if (this.activity is ProductTextsListActivity) {
        ...
        when (item.itemId) {
            R.id.save_item -> {
                ...
                if (operation == UIConstants.OP_CREATE) {
                    viewModel.create(productText)
                } else {
                    viewModel.update(productText)
                }
    ...
    private fun createProductText(): ProductText {
        val productText = ProductText(true)
        return productText
    }
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = ProducttextsCreateUpdateBinding.inflate(inflater, container, false)
        val rootView = binding.root
        // Bind ProductTexts data object to UI
        binding.setProductText(productText)
        return rootView
    }
    ...
    private fun setActivityTitle() {
        ...
        if (activity is ProductTextsListActivity) {
     ...
     */
    private fun handleSuccessfulCompletion(result: OperationResult<ProductText>) {
    ...
    /**
     * Restarts ProductTextsDetailFragment with the updated ProductText entity instance
     *
     * @param [productText] updated productText returned
     */
    private fun restartDetailFragment(productText: ProductText?) {
    ...
    companion object {
        private val LOGGER = LoggerFactory.getLogger(ProductTextsCreateFragment::class.java)
    }
}
