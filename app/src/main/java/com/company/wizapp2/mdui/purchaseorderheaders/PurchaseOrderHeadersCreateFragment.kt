...
import com.company.wizapp2.databinding.PurchaseorderheadersCreateUpdateBinding
...
import kotlinx.android.synthetic.main.purchaseorderheaders_create_update.*
 ...
 */
class PurchaseOrderHeadersCreateFragment : Fragment() {
    /* purchaseOrderHeader to be created or updated */
    private lateinit var purchaseOrderHeader: PurchaseOrderHeader

    /* DataBinding generated class */
    private lateinit var binding: PurchaseorderheadersCreateUpdateBinding
    ...
    private val isPurchaseOrderHeaderValid: Boolean
        get() {
            val linearLayout: LinearLayout = create_update_purchaseorderheader
            var isValid = true
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            ...
            purchaseOrderHeader = if (UIConstants.OP_CREATE == operation) {
        ...
        when (item.itemId) {
            R.id.save_item -> {
                ...
                if (operation == UIConstants.OP_CREATE) {
                    viewModel.create(purchaseOrderHeader)
                } else {
                    viewModel.update(purchaseOrderHeader)
                }
    ...
    private fun createPurchaseOrderHeader(): PurchaseOrderHeader {
        val purchaseOrderHeader = PurchaseOrderHeader(true)
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = PurchaseorderheadersCreateUpdateBinding.inflate(inflater, container, false)
        val rootView = binding.root
        // Bind PurchaseOrderHeaders data object to UI
    ...
    private fun setActivityTitle() {
        ...
        if (activity is PurchaseOrderHeadersListActivity) {
     ...
     */
    private fun handleSuccessfulCompletion(result: OperationResult<PurchaseOrderHeader>) {
    ...
    /**
     * Restarts PurchaseOrderHeadersDetailFragment with the updated PurchaseOrderHeader entity instance
     *
     * @param [purchaseOrderHeader] updated purchaseOrderHeader returned
     */
    private fun restartDetailFragment(purchaseOrderHeader: PurchaseOrderHeader?) {
    ...
    companion object {
        private val LOGGER = LoggerFactory.getLogger(PurchaseOrderHeadersCreateFragment::class.java)
    }
}
