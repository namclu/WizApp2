...
import com.company.wizapp2.databinding.SalesorderitemsDetailBinding
...
/**
 * A fragment representing a single SalesOrderItem detail screen. This fragment is either contained in an
 * SalesOrderItemsListActivity in two-pane mode (on tablets) or an SalesOrderItemsDetailActivity (on phone).
 */
class SalesOrderItemsDetailFragment : Fragment() {
    /* Generated data binding class based on layout file */
    private lateinit var binding: SalesorderitemsDetailBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if (bundle != null && bundle.containsKey(BundleKeys.ENTITY_INSTANCE)) {
            salesOrderItem = bundle.getParcelable(BundleKeys.ENTITY_INSTANCE)!!
            if (this.activity is SalesOrderItemsListActivity) {
                activityTitle = salesOrderItem.entityType.localName
                activity.invalidateOptionsMenu()
            } else {
                activity.title = salesOrderItem.entityType.localName
            }
            setupObjectHeader(salesOrderItem)
        }

        // In two pane mode, SalesOrderItemsListActivity is already receiving delete completion callback
    ...
    fun onNavigationClickedToSalesOrderHeaders_Header(view: View) {
        ...
        intent.putExtra("parent",  salesOrderItem)
     ...
     *
     * @param [salesOrderItem] to be displayed in ObjectHeader
     */
    private fun setupObjectHeader(salesOrderItem: SalesOrderItem) {
        ...
        objectHeader?.subheadline = EntityKeyUtil.getOptionalEntityKey(salesOrderItem)
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = SalesorderitemsDetailBinding.inflate(inflater, container, false)
