...
import com.company.wizapp2.databinding.ProductcategoriesDetailBinding
...
/**
 * A fragment representing a single ProductCategory detail screen. This fragment is either contained in an
 * ProductCategoriesListActivity in two-pane mode (on tablets) or an ProductCategoriesDetailActivity (on phone).
 */
class ProductCategoriesDetailFragment : Fragment() {
    /* Generated data binding class based on layout file */
    private lateinit var binding: ProductcategoriesDetailBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if (bundle != null && bundle.containsKey(BundleKeys.ENTITY_INSTANCE)) {
            productCategory = bundle.getParcelable(BundleKeys.ENTITY_INSTANCE)!!
            if (this.activity is ProductCategoriesListActivity) {
                activityTitle = productCategory.entityType.localName
                activity.invalidateOptionsMenu()
            } else {
                activity.title = productCategory.entityType.localName
            }
            setupObjectHeader(productCategory)
        }

        // In two pane mode, ProductCategoriesListActivity is already receiving delete completion callback
     ...
     *
     * @param [productCategory] to be displayed in ObjectHeader
     */
    private fun setupObjectHeader(productCategory: ProductCategory) {
        ...
        objectHeader?.subheadline = EntityKeyUtil.getOptionalEntityKey(productCategory)
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = ProductcategoriesDetailBinding.inflate(inflater, container, false)
