...
import com.company.wizapp2.databinding.ProductsDetailBinding
...
/**
 * A fragment representing a single Product detail screen. This fragment is either contained in an
 * ProductsListActivity in two-pane mode (on tablets) or an ProductsDetailActivity (on phone).
 */
class ProductsDetailFragment : Fragment() {
    /* Generated data binding class based on layout file */
    private lateinit var binding: ProductsDetailBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if (bundle != null && bundle.containsKey(BundleKeys.ENTITY_INSTANCE)) {
            product = bundle.getParcelable(BundleKeys.ENTITY_INSTANCE)!!
            if (this.activity is ProductsListActivity) {
                activityTitle = product.entityType.localName
                activity.invalidateOptionsMenu()
            } else {
                activity.title = product.entityType.localName
            }
            setupObjectHeader(product)
        }

        // In two pane mode, ProductsListActivity is already receiving delete completion callback
    ...
    fun onNavigationClickedToStock_StockDetails(view: View) {
        val intent = Intent(this.activity, StockListActivity::class.java)
        intent.putExtra("parent",  product)
     ...
     *
     * @param [product] to be displayed in ObjectHeader
     */
    private fun setupObjectHeader(product: Product) {
        ...
        objectHeader?.subheadline = EntityKeyUtil.getOptionalEntityKey(product)
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = ProductsDetailBinding.inflate(inflater, container, false)
